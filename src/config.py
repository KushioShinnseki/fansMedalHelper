...
class Config:
    ...
    def _validate_config(self, users: Dict[str, Any]) -> None:
        ...
        validations = [
            ...
            ("LIKE_CD", users.get("LIKE_CD"), lambda x: x >= 0, "LIKE_CD参数错误，必须>=0"),
            ("DANMAKU_CD", users.get("DANMAKU_CD"), lambda x: x >= 0, "DANMAKU_CD参数错误，必须>=0"),
            ("DANMAKU_NUM", users.get("DANMAKU_NUM"), lambda x: x >= 0, "DANMAKU_NUM参数错误，必须>=0"),
            ("WATCHINGLIVE", users.get("WATCHINGLIVE"), lambda x: x >= 0, "WATCHINGLIVE参数错误，必须>=0"),
            ("WEARMEDAL", users.get("WEARMEDAL"), [0, 1], "WEARMEDAL参数错误，必须为0或1"),
            ("WATCHINGALL", users.get("WATCHINGALL", 0), [0, 1], "WATCHINGALL参数错误，必须为0或1"),
        ]
